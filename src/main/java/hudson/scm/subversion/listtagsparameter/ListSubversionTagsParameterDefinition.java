 ...
 *
 *******************************************************************************/
package hudson.scm.subversion.listtagsparameter;
...
/**
 * Defines a new {@link ParameterDefinition} to be displayed at the top of the configuration page of {@link AbstractProject}s.
 * <p/>When used, this parameter will request the user to select a Subversion tag at build-time by displaying a
 * drop-down list. See {@link ListSubversionTagsParameterValue}.
 * <p/>
 *
...
public class ListSubversionTagsParameterDefinition extends ParameterDefinition
    ...
    public List<String> getTags() {
        ...
        for (AbstractProject project : jobs) {
            ParametersDefinitionProperty property = (ParametersDefinitionProperty) project.getProperty(
        ...
        DirectoriesSvnEntryHandler dirEntryHandler = new DescendingRevisionSortedSVNDirEntryHandler();
        ...
        try {
            ...
            LOGGER.log(Level.SEVERE, "An SVN exception occurred while listing the directory entries at " + getTagsDir(),
                e);
            return new ArrayList() {

                {
                    add("&lt;"
                        + ResourceBundleHolder.get(ListSubversionTagsParameterDefinition.class).format("SVNException")
                        + "&gt;");
                }
            };
        }

        List<String> dirs = dirEntryHandler.getDirectoryNames();

        // SVNKit's doList() method returns also the parent dir, so we need to remove it
        if (dirs != null) {
            removeParentDir(dirs);
        } else {
            LOGGER.log(Level.INFO, "No directory entries were found for the following SVN repository: {0}",
                getTagsDir());
            return new ArrayList() {

                {
                    add("&lt;" + ResourceBundleHolder.get(ListSubversionTagsParameterDefinition.class)
    ...
    public static class DescriptorImpl extends ParameterDescriptor {
        ...
        public FormValidation doCheckTagsDir(StaplerRequest req, @AncestorInPath AbstractProject context,
